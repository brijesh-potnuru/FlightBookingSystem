...
public class ApiGatewayConfig {
	...
	public RouteLocator myRoutes(RouteLocatorBuilder routeLocatorBuilder)
	{
		return routeLocatorBuilder.routes()
				.route(p ->p
                        .path("/fare/Access/findAll")
                        .uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/find/{flightNo}")
						.uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/add")
						.uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/update/{flightNo}")
						.uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/delete/{flightNo}")
						.uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/updateSeats/{flightNo}/{noOfPassengers}")
						.uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/findBy/{startPoint}/{endPoint}/{flightDate}")
						.uri("http://localhost:8081"))
				.route(p ->p
						.path("/fare/Access/signup")
						.uri("http://localhost:8081"))
		       .build();
	}
